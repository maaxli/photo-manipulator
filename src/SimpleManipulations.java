...
public class SimpleManipulations {
    ...
    public static PixelPicture border(PixelPicture pic, int borderWidth, Pixel borderColor) {
        ...
        Pixel[][] src = pic.getBitmap();
        Pixel[][] res = new Pixel[h + 2 * borderWidth][w + 2 * borderWidth];
        for (int i = 0; i < res.length; i++) {
            for (int j = 0; j < res[i].length; j++) {
                if (i < borderWidth || i >= h + borderWidth ||
                    j < borderWidth || j >= w + borderWidth) {
                    res[i][j] = borderColor;
                } else {
                    res[i][j] = src[i - borderWidth][j - borderWidth];
                }
            }
        }
    ...
    public static PixelPicture invertColors(PixelPicture pic) {
        ...
        for (int i = 0; i < h; i++) {
            for (int j = 0; j < w; j++) {
                ...
                int g = p.getGreen();
                int b = p.getBlue();
                src[i][j] = new Pixel(255 - r, 255 - g,255 - b);
            }
        }
    ...
    public static PixelPicture grayScaleAverage(PixelPicture pic) {
        ...
        for (int i = 0; i < h; i++) {
            for (int j = 0; j < w; j++) {
                ...
                int x = (int) Math.round(avg);
                src[i][j] = new Pixel(x, x, x);
            }
        }
    ...
    public static PixelPicture scaleColors(
        ...
        for (int i = 0; i < h; i++) {
            for (int j = 0; j < w; j++) {
                Pixel p = src[i][j];
                double rDouble = rfactor * p.getRed();
                int rRounded = (int) Math.round(rDouble);
                double gDouble = gfactor * p.getGreen();
                int gRounded = (int) Math.round(gDouble);
                double bDouble = bfactor * p.getBlue();
                int bRounded = (int) Math.round(bDouble);

                int r = Math.max(0, Math.min(255, rRounded));
                int g = Math.max(0, Math.min(255, gRounded));
                int b = Math.max(0, Math.min(255, bRounded));

                src[i][j] = new Pixel(r, g, b);
            }
        }
    ...
    public static PixelPicture alphaBlend(double alpha, PixelPicture pic, PixelPicture f) {
        ...
        Pixel[][] src1 = pic.getBitmap();
        Pixel[][] src2 = f.getBitmap();
        for (int i = 0; i < h1; i++) {
            for (int j = 0; j < w1; j++) {
                Pixel p1 = src1[i][j];
                Pixel p2 = src2[i][j];
                ...
                int rNew = weightedAverage(alpha, r1, r2);
                int gNew = weightedAverage(alpha, g1, g2);
                int bNew = weightedAverage(alpha, b1, b2);

                src1[i][j] = new Pixel(rNew, gNew, bNew);
            }
        }

        return new PixelPicture(src1);
    }
